...
def get_data_for_training(x, y, end=40, step=4, batch_size=1000, shuffle=True):
    x_data = x[0:end:step]
    y_data = y[0:end:step]
    train_dataloader = DataLoader(list(zip(x_data, y_data)), batch_size=batch_size, shuffle=shuffle)
...
def get_x_physics(x, n=30):
    ...
    x_physics = torch.linspace(lower_bound, upper_bound, n).view(-1,1).requires_grad_(True)
    return x_physics


def train(x, y, model, optimizer, epochs, data_loss_fn=nn.MSELoss(), physics_loss_fn=None, physics_loss_coef=None, save_plot=True, draw_loss=True):
